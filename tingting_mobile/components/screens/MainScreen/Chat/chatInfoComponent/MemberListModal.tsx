...
const MemberListModal: React.FC<Props> = ({
  ...
  // Open confirmation modal for removing a member
  const openConfirmModal = useCallback((userId: string) => {
    setMemberToRemove(userId);
    setShowConfirmModal(true);
  }, []);

  // Confirm removal of a member
  ...
  const handleRemoveMember = useCallback(
    async (memberIdToRemove: string) => {
      ...
      if (currentUserId === memberIdToRemove) {
        Alert.alert('Lỗi', 'Bạn không thể tự xóa mình khỏi nhóm. Hãy rời nhóm từ trang thông tin nhóm.');
        setRemoving(false);
        return;
      }
      ...
      setRemoving(true);
      try {
        removeParticipant(socket, { conversationId: chatInfo._id, userId: memberIdToRemove }, (response: any) => {
          setRemoving(false);
          if (response.success) {
        ...
        Alert.alert('Lỗi', 'Lỗi không xác định khi xóa thành viên.');
        setRemoving(false);
      }
    },
    [socket, isAdmin, currentUserId, chatInfo._id, onMemberRemoved]
  );

  // Handle clicking on a member to start a conversation
  const handleMemberClick = useCallback(
      ...
      try {
        ...
        if (res?.conversationId) {
          const messageData = {
            ...
            isGroup: false,
            participants: [{ userId: currentUserId }, { userId: memberId }],
          };
    ...
    [currentUserId, dispatch, navigation, onClose]
  );
                ...
                <View style={styles.memberItem}>
                  ...
                  {isAdmin && currentUserId !== item.userId && (
                    <TouchableOpacity
                      ...
                      disabled={removing}
                    >
                      <Ionicons name="trash" size={16} color={removing ? '#ccc' : '#ff0000'} />
                    </TouchableOpacity>
                  )}
                </View>
              )}
