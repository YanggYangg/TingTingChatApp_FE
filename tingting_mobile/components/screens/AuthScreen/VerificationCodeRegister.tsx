...
type VerificationCodeRegisterProps = NativeStackScreenProps<RootStackParamList, "VerificationCodeRegister">

const VerificationCodeRegister: React.FC<{ navigation: any; route: any }> = ({ navigation, route }) => {
  const { phoneNumber = "(+84) 372 374 650" } = route.params || {}
  const [VerificationCodeRegister, setVerificationCodeRegister] = useState<string[]>(Array(6).fill(""))
  ...
  const handleKeyPress = (e: any, index: number) => {
    ...
    if (e.nativeEvent.key === "Backspace" && index > 0 && !VerificationCodeRegister[index]) {
      inputRefs.current[index - 1]?.focus()
    }
  }
