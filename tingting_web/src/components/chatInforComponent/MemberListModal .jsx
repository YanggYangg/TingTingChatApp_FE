    ...
    >
      ...
      <ul className="max-h-60 overflow-y-auto">
        {chatInfo.participants.map((member, index) => (
          <li key={member._id} className="py-2 border-b last:border-none flex items-center">
            <div className="w-10 h-10 flex items-center justify-center bg-gray-300 rounded-full mr-3">
              <span className="text-gray-700 text-sm">{member.userId.slice(-4)}</span>
            </div>
            <div>
              <p className="text-gray-800">ID: {member.userId}</p>
