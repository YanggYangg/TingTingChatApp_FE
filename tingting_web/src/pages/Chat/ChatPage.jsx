...
function ChatPage() {
  ...
  const selectedMessageId = selectedMessage?.id; // Lấy id từ selectedMessage

  // Cuộn xuống tin nhắn mới khi messages thay đổi
  useEffect(() => {
  ...
  useEffect(() => {
    if (socket && selectedMessageId) {
      // Tham gia cuộc trò chuyện với conversationId
      socket.emit("joinConversation", selectedMessageId);

      // Lắng nghe tin nhắn đã lưu
      ...
      // Cleanup khi component unmount hoặc selectedMessageId thay đổi
      return () => {
    ...
    ? {
        ...
        name:
          selectedMessage.participants?.find((p) => p.userId !== currentUserId)
            ...
            ? "Bạn"
            : "Unknown", // Có thể thay bằng API để lấy tên thực
        avatar: "https://picsum.photos/200", // Mặc định, có thể lấy từ API
                  ...
                  .map((msg) => (
                    <MessageItem
                      ...
                      msg={{
                        ...
                        content: msg.content || "",
                        linkURL: msg.linkURL || "",
                        userId: msg.userId, // Đảm bảo userId được truyền
                      }}
