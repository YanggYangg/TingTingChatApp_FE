...
            return {
              ...msg,
              lastMessage: lastMessage?.content || "", // Sử dụng content từ lastMessage hoặc rỗng nếu null
              lastMessageType: lastMessage?.messageType || "text", // Sử dụng messageType hoặc mặc định là "text"
              lastMessageSenderId: lastMessage?.userId || null, // Sử dụng userId hoặc null
              time:  "", // Cập nhật thời gian nếu có lastMessage
              updateAt: updatedAt || new Date().toISOString(), // Sử dụng updatedAt từ payload
            };
          }
          return msg;
        })
      );
      dispatch(setLastMessageUpdate({ conversationId, lastMessage })); // Dispatch để đồng bộ Redux
...
          <button
            className={`font-semibold px-2 ${selectedTab === "priority" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-600"
              }`}
...
          </button>
          <button
            className={`font-semibold px-2 ${selectedTab === "others" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-600"
              }`}
